...
namespace FovUtils
{
	...
	struct FVec2
	{
		...
		FVec2(const float X, const float Y) : X(X), Y(Y)
		{
		}
	};
	...
	// ********************************
	inline float SinLerp(const float T)
	{
		return sinf(T * CONST_PI * 0.5f);
	}

	inline float CosLerp(const float T)
	{
		return 1.0f - cosf(T * CONST_PI * 0.5f);
	}
	...
	inline float SmootherStep(const float T)
	{
	...
	inline float TanDeg(const float A)
	{
		return tanf(CONST_PI / 180.0f * A);
	}

	inline float AtanDeg(const float A)
	{
		return 180.0f / CONST_PI * atanf(A);
	}
...
#define ALIGN_256(MemorySize) (((MemorySize) + 255) & ~255)
	...
	// ***********************************
	inline float HorFovToVertFov(const float HorizontalFov, const float Width, const float Height)
	{
	...
	inline float HorFovToHorPlus(float TargetHorizontalFov, float TargetWidth, float TargetHeight, float CurrentWidth,
	                             float CurrentHeight)
	{
	...
	inline ResolutionInformation GetMaxFittingResolution(float TargetWidth, float TargetHeight, float CurrentWidth,
	                                                     float CurrentHeight)
	{
